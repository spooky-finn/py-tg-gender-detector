...
from cfg.cfg import api_hash, api_id
...
from .startupMessage import readme_data, startup_message
...
async def main(application_path, path_to_session):
    async with TelegramClient(path_to_session, api_id, api_hash) as client:
        ...
        with open(os.path.join(application_path, "README.txt"), "w") as m:
            print(readme_data, file=m)
        ...
        if target == 2:
            ...
            a = input("Введите путь до папки с файлами .json: ")
            oshib_param = input(
                "Какой параметр ошибочный? 'system_lang_pack' или 'app_version' или 'app_id': "
            )
            path_to_photos = a
            valid_files = ".json"
            ost_elem = []
            ...
            if oshib_param == "app_id":
                # Читаем из файла и убираем \n если они тут есть
                with open("app_id_normal.txt", "r") as file:
                    ost_elem = file.readlines()
                print(ost_elem)
                for i in range(len(ost_elem)):
                    if "\n" in ost_elem[i]:
                        ost_elem[i] = ost_elem[i][:-1]
                ...
                if oshib_param == "app_version":
                    if ost_elem != []:
                        if str(line["app_version"]) not in ost_elem:
                            ...
                            try:
                                os.replace(
                                    file_source + "/" + line["session_file"] + ".json",
                                    file_destination
                                    + "/"
                                    + line["session_file"]
                                    + ".json",
        ...
        if target == 6:
            ...
            a = input("Введите путь до папки с файлами .json: ")
            min_kolvo_message = int(
                input(
                    "Введите мин. кол-во сообщений, которое должен был отправить аккаунт: "
                )
            )
                        ...
                        if (len(str(stroki[i + 1]))) >= 2:
                            if stroki[i + 1][1] not in [
                                ...
                                if kolvo_message_po_factu >= min_kolvo_message:
                                    path = os.path.join(
                                        path_to_photos, str(stroki[i]) + ".json"
                                    )
                                    with open(path, "r") as f:
                                        line = f.readlines()
                                    line = json.loads(line[0])
                                ...
                                if stroki[i + 1][1] in [
                                    ...
                                    kolvo_message_po_factu = int(
                                        stroki[i + 1][0] + stroki[i + 1][1]
                                    )
                                    if kolvo_message_po_factu >= min_kolvo_message:
                                        path = os.path.join(
                                            path_to_photos, str(stroki[i]) + ".json"
                                        ...
                                        line = json.loads(line[0])
                                        lang_pack.append(line["lang_pack"])
                                        system_lang_pack.append(
                                            line["system_lang_pack"]
                                        )
                                        app_version.append(line["app_version"])
                                        app_id.append(line["app_id"])
                                        app_hash.append(line["app_hash"])
                                        device.append(
                                            line["device"] + ":" + line["sdk"]
                                        )
                                        first_name.append(line["first_name"])
                                        last_name.append(line["last_name"])
        ...
        if target == 10:
            await cExcluder.FullUserIncluder()
        if target == 11:
...
def run(WORKDIR):
    ...
    asyncio.run(main(application_path, path_to_session))
