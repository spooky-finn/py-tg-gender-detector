...
print("Для начала работы прочитайте памятку! Она поможет вам понять, как работает софт и не совершить ошибок\n")

chto_nujno = int(input("Что бы вы хотели сделать?"
                   "\n"
                   ...
                   "4 - Хочешь разделить username и id по разным файлам?\n"
                   "5 - Хочешь найти уникальных юзеров среди чатов конкурентов(ту аудиторию, которой нет у тебя?)\n"
                   "6 - Удалить весь накопившийся мусор из папки, кроме самого скрипта\n"
...
if chto_nujno == 6:
    ...
    for i in range(len(file_source)-4):
        if file_source[i] == "\\":
            file_source = file_source[:i] + "/" + file_source[i+1:]
    print(file_source)
    get_files = os.listdir(file_source)
